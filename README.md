...
GitHub Pages solo sirve archivos estáticos: **no puede enviar correo por sí mismo**. El formulario necesita un servicio externo que reciba el mensaje y lo reenvíe a jose@ownexis.com.
...
1. Crear una cuenta en [formspree.io](https://formspree.io) y un formulario nuevo con destino `jose@ownexis.com`.
...
**Limpieza:** eliminar el registro A `@` de la página aparcada de GoDaddy y el CNAME `www` → `@` si existe. **No tocar los registros MX ni TXT**: si el correo `jose@ownexis.com` se gestiona desde GoDaddy o Microsoft 365, borrarlos deja el correo sin servicio.
